...
exports.sendAlarm = functions.https.onRequest(async (req, res) => {
    ...
    try {
        ...
        // 뿌리오 API 호출
        const response = await fetch('https://message.ppurio.com/api/send', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': process.env.PPURIO_API_KEY
            },
            body: JSON.stringify({
                userId: process.env.PPURIO_USER_ID,
                phone,
                templateId,
                variables
            })
